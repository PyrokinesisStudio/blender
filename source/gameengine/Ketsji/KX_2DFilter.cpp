...
bool KX_2DFilter::CheckTexture(int index, int bindCode, const std::string& prefix)
{
	if (!m_shader) {
	...
	if (index < 0 || index >= RAS_Texture::MaxUnits) {
		PyErr_Format(PyExc_ValueError, "%s: KX_2DFilter, index out of range [0, 7]", prefix.c_str());
		return false;
	}
	if (bindCode < 0) {
